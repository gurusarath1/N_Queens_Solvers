...
# this function generates the next queen postions on the board sequentially
def iterateQueensOnBoard(ChessBroard_Encoded, QueenRow, N):

	if ChessBroard_Encoded[QueenRow] == N - 1:
		ChessBroard_Encoded[QueenRow] = 0
		QueenRow = QueenRow + 1
		if QueenRow < N:
			iterateQueensOnBoard(ChessBroard_Encoded, QueenRow, N)
		else:
			return
	else:
		ChessBroard_Encoded[QueenRow] += 1
		return
...
# Last state (Queen Postions generated by iterateQueensOnBoard function)
last = [N-1 for n in range(N)]

# Iterate through all the queen positions
while ChessBroard_Encoded != last:
	iterateQueensOnBoard(ChessBroard_Encoded, 0, N)
	if GoalTest(ChessBroard_Encoded):
		NumberOfSolutions = NumberOfSolutions + 1
		print()
		print('Solution:',NumberOfSolutions, '___________________________')
		print(ChessBroard_Encoded)
		printChessBoard(ChessBroard_Encoded, N)
